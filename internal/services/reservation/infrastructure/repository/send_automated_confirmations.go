...
func (r *ReservationRepository) SendAutomatedConfirmations(ctx context.Context, branchId int32) (res bool, err error) {
	...
	if _, err := r.SendBulkReservationWhatsappConfirmation(ctx, reservationIds, branchId); err != nil {
		logger.Default().Errorf("Failed to send bulk reservation whatsapp confirmation: %v", err)
		return false, err
	}
